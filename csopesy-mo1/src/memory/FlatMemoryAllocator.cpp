...
FlatMemoryAllocator::FlatMemoryAllocator(std::uint64_t totalSize) : totalSize(totalSize) {
    blocks.push_back({0, totalSize, true, ""});
}

std::optional<std::uint64_t> FlatMemoryAllocator::allocate(std::uint64_t size, const std::string& owner) {
...
void FlatMemoryAllocator::deallocate(const std::string& owner) {
    std::lock_guard<std::mutex> lock(mtx);
    for (auto& b : blocks) {
...
bool FlatMemoryAllocator::admit(Process& proc, std::uint64_t size) {
...
bool FlatMemoryAllocator::handleFault(Process&, std::uint64_t) {
    return true;
}

bool FlatMemoryAllocator::isDemandPaged() const {
    return false;
}
...
std::uint64_t FlatMemoryAllocator::getExternalFragmentation() const {
    std::lock_guard<std::mutex> lock(mtx);
    std::uint64_t frag = 0;
...
void FlatMemoryAllocator::writeSnapshot(int quantumIndex, const std::string& outDir) const {
...
std::uint64_t FlatMemoryAllocator::usedBytes() const {
    return totalSize - getExternalFragmentation();
}

std::uint64_t FlatMemoryAllocator::freeBytes() const {
    return getExternalFragmentation();
}

std::uint64_t FlatMemoryAllocator::totalBytes() const {
    return totalSize;
}

std::uint64_t FlatMemoryAllocator::pagedIn() const {
    return 0;
}

std::uint64_t FlatMemoryAllocator::pagedOut() const {
    return 0;
}
