...
void Console::cmdSchedulerStart() {
    ...
    genThread = std::thread([this] {
        // Spec: generate one process every `batch-process-freq` CPU cycles. We track the
        // scheduler's shared CPU tick counter and admit a process each time it advances by
        // batchProcessFreq ticks (poll briefly to avoid a busy spin). The first process is
        // admitted unconditionally to bootstrap — ticks only advance once something runs.
