...
void RRScheduler::schedulerLoop() {
    while (true) {
        ...
        schedulerCv.wait(lock, [&] {
            ...
            return !running || (hasWork && hasFreeCore);
        });

        // Shutdown stops dispatching immediately — any processes still queued are
        // abandoned rather than drained, so program exit is prompt even under a large
        // backlog (workers finish only their current quantum). `scheduler-stop` does NOT
        // call stop(); it only halts generation, so queued work still completes normally.
        if (!running) break;
